...
static bool se3c_magic_init(se3_char* path, uint8_t* disco_buf, se3_discover_info* info)
{
    ...
    hfile.locked = false;
    hfile.fd = open((char*)mfpath, O_SYNC | O_RDWR | O_CREAT | __O_DIRECT | O_TRUNC, S_IWUSR | S_IRUSR);
...
static int se3c_open_existing(se3_char* path, bool rw, uint64_t deadline, se3_file* phfile)
{
    ...
    phfile->locked = false;
    if (rw) {
        fd = open(mfpath, O_SYNC | O_RDWR | __O_DIRECT, S_IWUSR | S_IRUSR);
    }
    else {
        fd = open(mfpath, O_SYNC | O_RDONLY | __O_DIRECT, S_IWUSR | S_IRUSR);
    }
    if (fd<0) {
        if (errno == ENOENT) {
