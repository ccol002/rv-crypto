 ...
 *  
 */
uint16_t L1_crypto_update(se3_session* s, uint32_t sess_id, uint16_t flags, uint16_t data0_len, uint8_t* data0, uint16_t data1_len, uint8_t* data1, uint16_t data2_len, uint8_t* data2, uint16_t* dataout_len, uint8_t* data_out);
...
/**
 *  \brief Encapsulation of a key with KEM
 *
 *  \param [in] s Pointer to current se3_session, you must be logged in
 *  \param [in] algorithm Which kem to use
 *  \param [in] version what KEM setting to use
 *  \param [out] pk_len How long is the public key buffer
 *  \param [out] pk Pointer to public key
 *  \param [in] sk_id ID of a secret key
 *  \return Error code or SE3_OK
 *
 */
 ...
 *  \param [in] algorithm Which kem to use
 *  \param [in] version what KEM setting to use
 *  \param [out] ct_len How long is the ciphertext buffer
 *  \param [out] ct Pointer to ciphertext output
 *  \param [out] ss_len How long is the shared secret buffer
 *  \param [out] ss Pointer to shared secret output
 *  \param [in] pk_len How long is the public key buffer
...
/**
 *  \brief Decapsulation of a key with KEM
 *
 *  \param [in] s Pointer to current se3_session, you must be logged in
 *  \param [in] algorithm Which kem to use
 *  \param [in] version what KEM setting to use
 *  \param [in] ct_len How long is the ciphertext buffer
 *  \param [in] ct Pointer to ciphertext input
 *  \param [out] ss_len How long is the shared secret buffer
 *  \param [out] ss Pointer to shared secret output
 *  \param [in] sk_id ID of stored secred key
 *  \return Error code or SE3_OK
 *
 */
