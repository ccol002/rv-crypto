...
uint16_t L1_getRandomFromDevice(se3_session* s, se3_device* dev, uint16_t access) {

	uint8_t sc[SE3_L1_CHALLENGE_SIZE];
	...
	FILE * fPtr;
		...
		for (int i =0;i<512*1024;i++){
				...
				memcpy(sc, session_data + SE3_CMD1_CHALLENGE_RESP_OFF_SC+1, SE3_L1_CHALLENGE_SIZE-2);


				// Prepare Challenge Response
				fwrite(sc,sizeof(sc[0]),SE3_L1_CHALLENGE_SIZE,fPtr);
		}
		/*
	for (int i=0;i<SE3_L1_CHALLENGE_SIZE;i++)
		fprintf(fPtr, "%c",sc[i]);
*/
...
uint16_t L1_crypto_update(se3_session* s,
	...
	SE3_SET32(session_data, SE3_CMD1_CRYPTO_UPDATE_REQ_OFF_DATAIN0_LEN, data0_len);   // Length of Data0
	...
	if (data0_len % 16 != 0) {
		data0_len_pad16 = data0_len + (16 - (data0_len % 16));
	}
	else data0_len_pad16 = data0_len;
	...
	data_len = SE3_CMD1_CRYPTO_UPDATE_REQ_OFF_DATA + data0_len_pad16 + data1_len_pad16 + data2_len;
	if (data_len > SE3_REQ1_MAX_DATA) {
		return SE3_ERR_PARAMS;
	}
	// copy data0
	if (data0_len > 0) {
		memcpy(session_data + SE3_CMD1_CRYPTO_UPDATE_REQ_OFF_DATA, data0, data0_len);
	}
	// copy data1
	if (data1_len > 0) {
		memcpy(session_data + SE3_CMD1_CRYPTO_UPDATE_REQ_OFF_DATA + data0_len_pad16, data1, data1_len);
	}
	//copy data2
	if (data2_len > 0) {
		memcpy(session_data + SE3_CMD1_CRYPTO_UPDATE_REQ_OFF_DATA + data0_len_pad16 + data1_len_pad16, data2, data2_len);
	}
...
uint16_t L1_kem_test(se3_session* s){
	uint16_t return_value = 0, resp_len = 0;
	uint8_t* session_data = s->buf + SE3_RESP1_OFFSET_DATA;
	uint16_t par=SE3_OK;
	SE3_SET32(session_data, SE3_CMD1_TEST_IMPLEMENTATIONS_REQ, par);

	return_value = L1_TXRX(s, SE3_CMD1_KEM_TEST, 0, SE3_CMD1_TEST_IMPLEMENTATIONS_REQ_SIZE, &resp_len);
	if (return_value != SE3_OK) {
		return return_value;
	}

	uint32_t num_of_algo;
	uint64_t u32tmp;
	// Read response
	SE3_GET32(session_data, SE3_CMD1_TEST_IMPLEMENTATIONS_RESP_NUM_OF_ALGO, num_of_algo);   // extract length of output data
	uint32_t offset=SE3_CMD1_TEST_IMPLEMENTATIONS_RESP_NUM_OF_ALGO+4;
	for(int i=0; i<num_of_algo-2;i++){
		SE3_GET32(session_data, offset, u32tmp);
		printf("ID: %ld\n",u32tmp);
		offset+=SE3_CMD1_TEST_IMPLEMENTATIONS_RESP_ALGO_ID_SIZE;

		printf("Algorithm_name: %s\n",session_data+offset);
		offset+=SE3_CMD1_TEST_IMPLEMENTATIONS_RESP_ALGO_NAME_SIZE;

		SE3_GET64(session_data, offset, u32tmp);
		printf("Keygen: %d\n",u32tmp);
		offset+=SE3_CMD1_TEST_IMPLEMENTATIONS_RESP_ALGO_TIMER_SIZE;
...
uint16_t L1_crypto_kem_keypair(se3_session* s, uint16_t algorithm, uint32_t key_valid,  uint32_t sk_id ,  uint32_t* pk_len, uint8_t* pk){
	uint16_t return_value = 0, resp_len = 0;
	uint16_t data_len = 0;
	uint16_t u16tmp;
	uint8_t* session_data = s->buf + SE3_RESP1_OFFSET_DATA;
	...
	SE3_SET16(session_data, SE3_CMD1_CRYPTO_KEM_KEYPAIR_REQ_OFF_ALGORITHM, algorithm);
	SE3_SET32(session_data, SE3_CMD1_CRYPTO_KEM_KEYPAIR_REQ_OFF_KEY_VALID, key_valid);
	SE3_SET32(session_data, SE3_CMD1_CRYPTO_KEM_KEYPAIR_REQ_OFF_SK_ID, sk_id);

	data_len = SE3_CMD1_CRYPTO_KEM_KEYPAIR_REQ_OFF_SK_ID + 4;
	return_value = L1_TXRX(s, SE3_CMD1_CRYPTO_KEM_KEYPAIR, 0, data_len, &resp_len);
	if (return_value != SE3_OK) {
		return return_value;
	}
	...
	// Read response
	SE3_GET32(session_data, SE3_CMD1_CRYPTO_KEM_KEYPAIR_RESP_OFF_PK_LEN, u16tmp);   // extract length of output data
	if (pk_len != NULL)
		*pk_len = u16tmp;
	if (pk != NULL)
		memcpy(pk, session_data + SE3_CMD1_CRYPTO_KEM_KEYPAIR_RESP_OFF_PK_DATA, u16tmp);   // extract output data

	return return_value;
}

uint16_t L1_crypto_kem_enc(se3_session* s, uint16_t algorithm, uint32_t* ct_len, uint8_t* ct, uint32_t* ss_len, uint8_t* ss, uint32_t pk_len, uint8_t* pk){
	uint16_t return_value = 0, resp_len = 0;
	uint16_t data_len = 0;
	uint16_t u16tmp;
	uint8_t* session_data = s->buf + SE3_RESP1_OFFSET_DATA;
	...
	SE3_SET16(session_data, SE3_CMD1_CRYPTO_KEM_ENC_REQ_OFF_ALGORITHM, algorithm);
	SE3_SET32(session_data, SE3_CMD1_CRYPTO_KEM_ENC_REQ_OFF_PK_LEN, pk_len);

	// copy data
	if (pk_len > 0) {
		memcpy(session_data + SE3_CMD1_CRYPTO_KEM_ENC_REQ_OFF_PK_DATA, pk, pk_len);
	}

	data_len = SE3_CMD1_CRYPTO_KEM_ENC_REQ_OFF_PK_DATA + pk_len;

	return_value = L1_TXRX(s, SE3_CMD1_CRYPTO_KEM_ENC, 0, data_len, &resp_len);
	if (return_value != SE3_OK) {
		return return_value;
	}
	...
	// Read response
	SE3_GET32(session_data, SE3_CMD1_CRYPTO_KEM_ENC_RESP_OFF_CT_LEN, u16tmp);   // extract length of output data
	if (ct_len != NULL)
		*ct_len = u16tmp;

	SE3_GET32(session_data, SE3_CMD1_CRYPTO_KEM_ENC_RESP_OFF_SS_LEN, u16tmp);   // extract length of output data
	if (ss_len != NULL)
		*ss_len = u16tmp;

	if (ct != NULL)
		memcpy(ct, session_data + SE3_CMD1_CRYPTO_KEM_ENC_RESP_OFF_DATA, *ct_len);   // extract output data
	if (ss != NULL)
		memcpy(ss, session_data + *ct_len + SE3_CMD1_CRYPTO_KEM_ENC_RESP_OFF_DATA, *ss_len);   // extract output data

	return return_value;
}

uint16_t L1_crypto_kem_dec(se3_session* s, uint16_t algorithm, uint32_t ct_len, uint8_t* ct, uint32_t* ss_len, uint8_t* ss,  uint32_t sk_id){
	uint16_t return_value = 0, resp_len = 0;
		uint16_t data_len = 0;
		uint16_t u16tmp;
		uint8_t* session_data = s->buf + SE3_RESP1_OFFSET_DATA;
		...
		SE3_SET16(session_data, SE3_CMD1_CRYPTO_KEM_DEC_REQ_OFF_ALGORITHM, algorithm);
		SE3_SET32(session_data, SE3_CMD1_CRYPTO_KEM_DEC_REQ_OFF_SK_ID, sk_id);
		SE3_SET32(session_data, SE3_CMD1_CRYPTO_KEM_DEC_REQ_OFF_CT_LEN, ct_len);

		// copy data
		if (ct_len > 0) {
			memcpy(session_data + SE3_CMD1_CRYPTO_KEM_DEC_REQ_OFF_CT_DATA, ct, ct_len);
		}

		data_len = SE3_CMD1_CRYPTO_KEM_DEC_REQ_OFF_CT_DATA + ct_len ;

		return_value = L1_TXRX(s, SE3_CMD1_CRYPTO_KEM_DEC, 0, data_len, &resp_len);
		if (return_value != SE3_OK) {
			return return_value;
		}
		...
		// Read response
		SE3_GET32(session_data, SE3_CMD1_CRYPTO_KEM_DEC_RESP_OFF_SS_LEN, u16tmp);   // extract length of output data
		if (ss_len != NULL)
			*ss_len = u16tmp;

		if (ss != NULL)
			memcpy(ss, session_data + SE3_CMD1_CRYPTO_KEM_DEC_RESP_OFF_SS_DATA, u16tmp);   // extract output data
...
uint16_t L1_encrypt(se3_session* s, uint16_t algorithm, uint16_t mode, uint32_t key_id, size_t datain_len, int8_t* data_in, size_t* dataout_len, uint8_t* data_out) {
	...
	do {
		if (datain_len - curr_chunk)
			error = L1_crypto_update(s, enc_sess_id, mode, 0,NULL,0, NULL, (uint16_t)curr_chunk, sp, &curr_len, rp);
		else
			error = L1_crypto_update(s, enc_sess_id, SE3_CRYPTO_FLAG_FINIT | mode,0,NULL, 0, NULL, (uint16_t)curr_chunk, sp, &curr_len, rp);
...
uint16_t L1_digest(se3_session* s, uint16_t algorithm, size_t datain_len, int8_t* data_in, size_t* dataout_len, uint8_t* data_out) {
	...
	do {
		if (datain_len - curr_chunk)
			error = L1_crypto_update(s, enc_sess_id, 0, (uint16_t)curr_chunk, sp, 0,NULL,0, NULL, &curr_len, rp);
		else
			error = L1_crypto_update(s, enc_sess_id, SE3_CRYPTO_FLAG_FINIT, (uint16_t)curr_chunk, sp,0,NULL, 0, NULL, &curr_len, rp);
