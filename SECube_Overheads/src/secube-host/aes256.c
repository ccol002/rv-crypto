...
uint8_t gmac(B5_tAesCtx *ctx, const uint8_t *iv, size_t iv_len, const uint8_t *aad, size_t aad_len, uint8_t *tag){
...
int32_t B5_Aes256_Update (B5_tAesCtx *ctx, uint8_t *encData, uint8_t *clrData, int16_t nBlk)
{
    ...
    switch(ctx->mode) {
        ...
        {
        	if(aes_gcm_enc(ctx, ctx->InitVector, B5_AES_IV_SIZE, clrData, nBlk*B5_AES_BLK_SIZE, NULL, 0, encData)==-1)
        		return B5_AES256_RES_INVALID_MODE;
        	break;
        }
        case B5_AES256_GCM_DEC:
        {
        	if( aes_gcm_dec(ctx, ctx->InitVector, B5_AES_IV_SIZE, encData, nBlk*B5_AES_BLK_SIZE, NULL, 0, clrData)==-1)
        		return B5_AES256_RES_INVALID_MODE;
